...
def prepare_dataframe(co, dem, aki, neph, apache, dx, drug_dfs=None):
    ...
    # initialize egfr
    df['egfr'] = 142 * np.power(0.9938, df['age'])
    # calculate for women
    idxFemale = df['gender'] == 'Female'
    df.loc[idxFemale, 'egfr'] *= 1.012
    df.loc[idxFemale, 'egfr'] *= np.power( (df.loc[idxFemale, 'creatinine_baseline']/kappa_female).clip(upper=1), alpha_female)
    df.loc[idxFemale, 'egfr'] *= np.power( (df.loc[idxFemale, 'creatinine_baseline']/kappa_female).clip(lower=1), -1.200)
    
    # calculate for men
    # multiply by creat/potassium factor
    df.loc[~idxFemale, 'egfr'] *= np.power( (df.loc[~idxFemale, 'creatinine_baseline']/kappa_male).clip(upper=1), alpha_male)
    df.loc[~idxFemale, 'egfr'] *= np.power( (df.loc[~idxFemale, 'creatinine_baseline']/kappa_male).clip(lower=1), -1.200)
