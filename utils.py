...
def extract_adm_and_wk(d, drugname='vanco'):
    ...
    idxKeep0 = (
        (d['drugstartoffset'] >= (-12*60)) & (d['drugstartoffset'] <= (12*60))
    )
    idxKeep1 = (
        (d['drugstopoffset'] >= (-12*60)) & (d['drugstopoffset'] <= (12*60))
    )
    ...
    idxKeep0 = (
        (d['drugstartoffset'] >= (48*60)) & (d['drugstartoffset'] <= (168*60))
    )
    idxKeep1 = (
        (d['drugstopoffset'] >= (48*60)) & (d['drugstopoffset'] <= (168*60))
    )
...
def get_matched_groups(treat_um, control_um, seed=None):
    ...
    for apache_group in all_apache_groups:
        ...
        sample_num = len(treat_group) if len(treat_group) < len(
            control_group) else len(control_group)
...
def match_and_print_or(exposure, control, seed=None):
    ...
    get_odds_ratio(diseased_exposed, healthy_exposed,
                   diseased_nonexposed, healthy_nonexposed)
