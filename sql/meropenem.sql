DROP TABLE IF EXISTS vanco.meropenem;
CREATE TABLE vanco.meropenem AS
SELECT 
  patientunitstayid,
  CASE 
...
WHERE 
  (drughiclseqno IN (11254) OR LOWER(drugname) LIKE '%meropenem%' OR LOWER(drugname) LIKE '%merrem%')
