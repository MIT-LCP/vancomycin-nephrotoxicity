...
WITH apache AS(
  SELECT 
    ...
    CASE
      ...
      WHEN apachescore = -1 THEN '-1' -- -1 indicates that there was an error when calculating the APACHE score
...
demographics AS (
  ...
  LEFT JOIN `hst-953-2018.team_i.weight` w ON w.patientunitstayid = p.patientUnitStayID 
